...
*Last updated: 2026-09-13 · verified against `322eebe` on `claude/lod-override-testing-rhwq4f`*
...
| D16 | The LOD contract is stale, and it is brittle in a way that makes staleness likely. Stale: the recorded verdicts have not been re-recorded since the change that last moved them, so the gate exits non-zero on an unmodified checkout. Brittle: a change to the food economy small enough to leave the population totals alone still flips a large block of verdicts, and a gentler version of the same change flips as many — so the count does not scale with the size of the behaviour change, and the gate cannot distinguish "LOD fidelity got worse" from "the trajectory moved". A replacement measurement exists — the rate at which two tiers separate rather than where they end up — now scaled against floor and ceiling controls; it reports and does not yet gate, and the stale contract is untouched. The replacement has since been found to inherit this defect at its plateau: see D19. Figures in [`../changelog.md`](../changelog.md) | [tooling-and-tests.md](tooling-and-tests.md) |
...
| D19 | The divergence curve — the instrument built to replace D16's end-state gate — has no resolution at its plateau. Scaled against a ceiling control (one tier against itself on two seeds, i.e. two worlds with nothing in common), the Full-against-Minimal plateau reads at or *above* the ceiling, on the state scalar as well as the combined one. A fidelity measure must place "same world, coarser cadence" strictly below "different world entirely"; this one does not, so a tolerance placed on the plateau cannot separate lost fidelity from a different run — D16's defect in a new form. Only the growth window keeps headroom, and the fidelity curve's own seed-to-seed scatter has not been measured, so whether that headroom is usable is unestablished. The floor control is exact, so this is a property of the measure and not of its determinism. Figures in [`../changelog.md`](../changelog.md) | [tooling-and-tests.md](tooling-and-tests.md) |
