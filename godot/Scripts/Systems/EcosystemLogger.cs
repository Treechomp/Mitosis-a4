...
public sealed class EcosystemLogger : ISystem
{
    ...
    // ── Whole-run totals ──────────────────────────────────────────────────────
    // Every counter above is zeroed when its snapshot row is written, which makes them useless
    // for comparing one COMPLETE run against another — the last row of a CSV only covers the
    // final interval. These accumulate for the life of the logger and are never reset; they are
    // what LodDifferentialRunner diffs between a Full-tier and a Minimal-tier run of the same
    // scenario. They are also maintained regardless of the terrain/nutrition logging toggles, so
    // a harness reading them does not depend on how a scenario happens to configure its CSVs.
    private readonly Dictionary<int, int> _runBirths       = new();
    private readonly Dictionary<int, int> _runDeathsStarve = new();
    private readonly Dictionary<int, int> _runDeathsAge    = new();
    private readonly Dictionary<int, int> _runDeathsPred   = new();
    private readonly Dictionary<int, int> _runDeathsEnv    = new();
    private readonly Dictionary<int, int> _runKillsMade    = new();
    ...
    public IReadOnlyDictionary<int, int> RunDeathsPred   => _runDeathsPred;
    public IReadOnlyDictionary<int, int> RunDeathsEnv    => _runDeathsEnv;
    public IReadOnlyDictionary<int, int> RunKillsMade    => _runKillsMade;

    /// <summary>Moisture nudges applied by TerraformSystem/NestSystem/etc. over the whole run.</summary>
    public int RunTerraformNudges { get; private set; }

    /// <summary>Nudges that crossed a tile-class boundary (a subset of the above).</summary>
    public int RunTerraformShifts { get; private set; }

    public float RunNutritionConsumed { get; private set; }
    ...
    {
        // Run totals first: they are the LOD-differential harness's terraform-rate signal and
        // must not depend on whether this scenario happens to write a terrain CSV.
        RunTerraformNudges++;
        if (oldTile != newTile) RunTerraformShifts++;
    ...
    public void LogBirth(int speciesId, int entityId, float x, float y, string detail = "")
    {
        ...
        Increment(_runBirths, speciesId);
        var name = SpeciesRegistry.GetById(speciesId)?.Name ?? speciesId.ToString();
    ...
    public void LogAgeDeath(int speciesId, int entityId, float x, float y)
    {
        ...
        Increment(_runDeathsAge, speciesId);
        var name = SpeciesRegistry.GetById(speciesId)?.Name ?? speciesId.ToString();
    ...
    public void LogStarvation(int speciesId, int entityId, float x, float y)
    {
        ...
        Increment(_runDeathsStarve, speciesId);
        var name = SpeciesRegistry.GetById(speciesId)?.Name ?? speciesId.ToString();
                         ...
                         float x, float y)
    {
        ...
        Increment(_runDeathsPred, preySpeciesId);
        Increment(_runKillsMade, predatorSpeciesId);
    ...
    public void LogReproduction(int speciesId, int parentId, float x, float y, int offspringCount)
    {
        ...
        Increment(_runBirths, speciesId, offspringCount);
        var name = SpeciesRegistry.GetById(speciesId)?.Name ?? speciesId.ToString();
    ...
    public void LogEnvironmentDeath(int speciesId, int entityId, float x, float y, string cause)
    {
        ...
        Increment(_runDeathsEnv, speciesId);
        var name = SpeciesRegistry.GetById(speciesId)?.Name ?? speciesId.ToString();
    ...
    /// <summary>
    /// Close every writer. Also clears <see cref="Instance"/>: systems log through that static,
    /// and a harness that runs several simulations in one process would otherwise leave them
    /// writing into a disposed logger between runs.
    /// </summary>
    public void Close()
    {
