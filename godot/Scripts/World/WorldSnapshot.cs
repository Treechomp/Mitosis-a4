...
public static class WorldSnapshot
{
        ...
        Dictionary<TileType, int> hist)
    {
        ...
        int water    = Count(TileType.DeepWater, TileType.ShallowWater, TileType.River, TileType.Reef);
        int cold     = Count(TileType.Ice, TileType.Tundra, TileType.Taiga);
        int desert   = Count(TileType.Sand, TileType.Arid);
        ...
        AppendNiche(sb, "water (Fish/Shark/Croc/Penguin food)", Pct(water));
        AppendNiche(sb, "cold  (Penguin/Polar Bear/Arctic Fox/Musk Ox)", Pct(cold));
        AppendNiche(sb, "desert(Scorpion/Lizard/Camel/Snake)", Pct(desert));
        ...
        FlagNiche(sb, "desert",      Pct(desert));
        FlagNiche(sb, "open water",  Pct(water));
